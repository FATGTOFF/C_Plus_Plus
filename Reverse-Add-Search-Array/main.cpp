...
int main()
{
    ...
    // Use the dynamically array to ask the user the number of elements.
    std::cout << "How many elements would you like to have in the array?: ";
    std::cin >> arrElements;
    numbers = new int[arrElements]; // Allocate the memory based on the number of int elements provided by the user.
    ...
    // The following two lines are required to free the allocated memory back to the system.
    delete[] numbers;
    numbers = nullptr;
...
void initialArray(int num[], int size)
{
    ...
    {
	   std::cout << "Row # " << rows + 1 << " :";
	   num[rows] = randomChoice(0, 100);
	   std::cout << num[rows] << std::endl;
    }
    std::cout << "Finished inputting the numbers in the list\n";
}
...
{
    std::cout << "\nDisplaying the original order of the array...\n";
    for (int rows = 0; rows < size; rows++)
    {
	   std::cout << "Row # " << rows + 1 << " :";
	   std::cout << num[rows] << std::endl;
...
void searchList(const int num[], const int size, int value)
{
    ...
    for (int rows = 0; rows < size; rows++)
    {
	   ...
	   {
		  foundNumtimes++;
		  std::cout << "Found [" << value << "] on Row #" << rows + 1 << std::endl;
		  found = true;
	   }
    }
    ...
    if (found == false)
	   ...
	   std::cout << "Found [" << value << "] " << foundNumtimes << " times.\n";
}
