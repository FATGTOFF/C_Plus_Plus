...
void CBinaryFileReaderDlg::OnBnClickedButtonOpenFile()
{
   ...
   std::ifstream log_EabEvents_Input(dialogFileOpen.GetPathName(), std::ios::in | std::ios::binary);
   std::vector<unsigned char> buffer(std::istreambuf_iterator<char>(log_EabEvents_Input), {});
   ...
   if (IDOK == dialogFileSave.DoModal())
   {
      ...
      for (unsigned count = 0; count < buffer.size(); count++)
      {
         ...
         if (0 == count % std::stoi(numOfBytesPerLine_str))
         {
            csvFile << "\n";
         }

         csvFile << std::setw(2) << std::internal << std::setfill('0');
         csvFile << std::hex << std::uppercase << static_cast<int>(buffer.at(count)) << ',';
      }
...
{
   // I chose 2 since 0 or 1 is used for success/error.
   EndDialog(2);
   return;
}
